...
public class CryptoManager {
	...
	public static String caesarEncryption(String plainText, int key) {
		...
		for (char ch : plainText.toCharArray()) {
			result.append((char) (LOWER_RANGE + (ch - LOWER_RANGE + key) % RANGE));
		}
	...
	public static String bellasoEncryption (String plainText, String bellasoStr) {
		...
		bellasoStr = extendKey(plainText, bellasoStr);
		StringBuilder result = new StringBuilder();
		...
		for (int i = 0; i < plainText.length(); i++) {
			...
			int shiftedValue = plainChar + keyChar; // Shift using key char
			while (shiftedValue > UPPER_RANGE) shiftedValue -= RANGE; // Wrap-around

			result.append((char) shiftedValue);
		}
	...
	public static String bellasoDecryption(String encryptedText, String bellasoStr) {
		...
		bellasoStr = extendKey(encryptedText, bellasoStr);
		StringBuilder result = new StringBuilder();
		...
		for (int i = 0; i < encryptedText.length(); i++) {
			...
			int shiftedValue = cipherChar - keyChar; // Reverse shift using key char
			while (shiftedValue < LOWER_RANGE) shiftedValue += RANGE; // Wrap-around
